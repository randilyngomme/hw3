...
function selectGameWithPlayer($gid) {
    try {
        $conn = get_db_connection();
        $stmt = $conn->prepare("SELECT p.player_id,player_name,player_gender,platform,device,genre,profile_id FROM player p join profile pf on pf.player_id= p.player_id WHERE pf.game_id=?");
